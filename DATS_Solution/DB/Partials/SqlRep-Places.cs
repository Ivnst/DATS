  ...
  public partial class SqlRep
  {
    ...
    public void SavePlacesBySector(Sector sector, List<PlaceView> places)
    {
      ...
      using (TransactionScope scope = new TransactionScope())
      {
        ...
        //проверям ширину и высоту на значения
        if (maxCol > Consts.MaxSectorWidth || maxRow > Consts.MaxSectorHeight)
        {
          throw new InvalidOperationException("Получены некорректные данные! Размеры сектора выходят за допустимые значения!");
        }
        ...
        foreach (PlaceView pv in places)
        {
          if (placeMatrix[pv.RowPos][pv.ColPos] != null)
          {
            throw new InvalidOperationException("Получены некорректные данные! Некоторые места имеют одинаковую позицию!");
          }
